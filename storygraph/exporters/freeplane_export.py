# storygraph/exporters/freeplane_export.py
...
def export_freeplane(nodes, edges, out_path, scene_name="Scene"):
    """
    Export a Freeplane (.mm) file.
    Tree-based, lossy graph projection.
    """
    ...
    for nid, srcs in incoming.items():
        ...
        if len(srcs) > 1:
            print(
                f"[freeplane] WARNING: node {nid} has "
...
def _resolve_color(color):
    ...
    if color.startswith("#"):
        return color
    return FREEPLANE_PALETTE.get(color)
